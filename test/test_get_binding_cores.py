...
def compute_labels_test(label, core, metal_name, no_neighbors, coordinating_resis, resnums, resindices):
...
def compute_distance_matrices_test(dist_mat, resnums, resindices, core, no_neighbors, coordinating_resis):
    max_atoms = 4*((no_neighbors * 2 * coordinating_resis) + coordinating_resis)
    tol =10e-6
    ...
    row_index = 0
    column_index = 0
    for resnum1 in resindices:
        for atom1 in ['N', 'CA', 'C', 'O']:
            for resnum2 in resindices:
                for atom2 in ['N', 'CA', 'C', 'O']:
...
def permute_features_test(features, dist_mat, label, encoding, no_neighbors, coordinating_resis, resnums, resindices):
    observations = features['full_observations']
    labels = features['full_labels']
    resnum_permutations = features['resnum_permutations']
    resindex_permutations = features['resindex_permutations']
    ...
    counter = 0
    for perm_feature, perm_label, perm_resnums, perm_resindices in zip(observations, labels, resnum_permutations, resindex_permutations):
        counter += 1
        ...
        perm_resnums = list(perm_resnums)
        perm_resindices = list(perm_resindices)
        ...
        assert len(perm_resindices) == len(resindices) <= max_resis
        ...
        for resnum1 in resindices:
            for resnum2 in resindices:
                perm_atoms_i = atom_indices[perm_resindices.index(resnum1)]
                perm_atoms_j = atom_indices[perm_resindices.index(resnum2)]
                atoms_i = atom_indices[np.where(resindices == resnum1)[0][0]]
                atoms_j = atom_indices[np.where(resindices == resnum2)[0][0]]
...
def test_all():
    ...
    for pdb in pdbs:
        ...
        for core, name in zip(unique_cores, unique_names):
            ...
            full_dist_mat, binding_core_resindices, binding_core_resnums, label = compute_distance_matrices(core, name, no_neighbors, coordinating_resis)
            compute_distance_matrices_test(full_dist_mat, binding_core_resnums, binding_core_resindices, core, no_neighbors, coordinating_resis)
            compute_labels_test(label, core, name, no_neighbors, coordinating_resis, binding_core_resnums, binding_core_resindices)
            ...
            permute_features_test(features, full_dist_mat, label, encoding, no_neighbors, coordinating_resis, binding_core_resnums, binding_core_resindices)
